...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $resultado = calculadora($_POST['operador'],$_POST['numero1'],$_POST['numero2']);
}
...
<body>
    ...
    <div id="header_wrap" class="outer">
        <header class="inner">
            <h1 id="project_title">"02-CalculadoraSimple.php"</h1>
            <h2 id="project_tagline">SOlucions</h2>
        </header>
    </div>
    <br>
    <h1><mark>Ejercicio Utilitza la funció feta al exercisi 3 de funcions per fer una calculadora amb dos operands i un operador.</mark></h1><br>
    <?php
        if (isset($resultado)) {
            echo "$resultado<br/>";
        }
    ...
    <form method="POST">
        ...
        <input type="text" name="numero1" placeholder="<?php echo (isset($_GET["numero1"])) ? $_GET["numero1"] : 'Primer número'; ?>" required/>
        <select name="operador" required>
            <option value="+">Suma</option>
            <option value="-">Resta</option>
            <option value="x">Multiplicación</option>
        </select>
        <input type="text" name="numero2" placeholder="<?php echo (isset($_GET["numero2"])) ? $_GET["numero2"] : 'Segundo número'; ?>" required/> =
